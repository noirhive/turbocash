...
const Register: React.FC = () => {
  ...
  const onSubmit = async (data: RegisterFormData) => {
    setIsLoading(true);
    try {
      // Mock registration with user limits - replace with Supabase
      await new Promise(resolve => setTimeout(resolve, 1000));
      
      // Mock user limit check
      const currentUserCount = 4; // Mock current users
      if (currentUserCount >= 6) {
        toast.error('User limit reached. Please contact system administrator.');
        return;
      }
      ...
      <div className="max-w-md w-full space-y-8">
        <div className="text-center">
          ...
          <p className="mt-2 text-sm text-gray-600 dark:text-gray-400">
            Join TurboCash to manage your transactions
          </p>
        </div>
